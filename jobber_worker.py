...
class JobberWorker(JobberMQTTThreadedClient):
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        self._mqtt_client.subscribe("mqtt_jobber/workers/"+self._mqtt_client_my_id+"/contracts.json")
    ...
    def on_message(self, client, userdata, msg):
        ...
        if msg.topic == "mqtt_jobber/dispatch.json":
            ...
            if self._do_i_meet_job_criteria(payload["worker_criteria"]):
                ...
                self._mqtt_client.publish(jobber_topic_offers_path.format(offer_id=payload['offer_id']), repr(self))
            ...
            self._mqtt_client.subscribe(jobber_topic_workers_path.format(job_number=payload["job_number"]))
            self.send_heartbeat_for_job(payload["job_number"], 0)

        self._logger.info(self._mqtt_client_my_id+"@"+msg.topic + " " + str(payload))

    def _do_i_meet_job_criteria(self, criteria):
        return True

    def send_heartbeat_for_job(self, job_number, work_sequence):
        self._mqtt_client.publish(jobber_topic_dispatcher_path.format(job_number=job_number),
